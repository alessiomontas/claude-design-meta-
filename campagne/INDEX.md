...
| [`reel-standard-alberghiero/`](reel-standard-alberghiero/) | Acquisizione proprietari — reel Instagram/Facebook/TikTok | "Questa casa non esiste. Lo standard, sì.": il gancio dichiara che gli interni sono una simulazione, poi tre controlli concreti (cucina, lenzuola, bagno) definiscono il livello di esecuzione | Proprietari che confrontano gestori e non sanno su cosa distinguerli | Reel 9:16 da 29,3 s **con musica**, copertina, 6 scene editabili, caption e voce fuori campo | 🟡 **Non ancora verificata** — il `compliance-checker` e' stato interrotto due volte, va rilanciato prima di pubblicare |
| [`reel-catena-recensioni/`](reel-catena-recensioni/) | Acquisizione proprietari — reel Instagram/Facebook | "Le recensioni non sono un complimento": la catena PUNTEGGIO → POSIZIONE → VISIBILITÀ → FIDUCIA → PRENOTAZIONI → PREZZO, poi "non è fortuna, è metodo, è lavoro" | Proprietari con annuncio già attivo e buone recensioni ma calendario vuoto in bassa stagione | Reel 9:16 su template animato Canva (`DAHVxjBJbDU`), 32 slot di testo sostituiti | 🔴 **Bloccata** — compliance chiusa su 4 punti su 5; restano i video stock di ville di lusso, da sostituire con riprese reali prima di pubblicare |

## Note

- **`gestione-case-vacanza-proprietari/`**: i documenti di processo iniziali (`brief-mercato.md`, `checklist-compliance.md`, `revisione-marketing-design.md`, `copy.md`) fotografano una versione precedente della campagna (con case study "Rome Smart Sea" e "hotel-style", poi corretti) e **non sono stati riallineati**. I contenuti effettivamente pubblicati/pubblicabili sono in `grafiche/`, `instagram-ready/out/`, `direzione-artistica.md`, `PIANO-PUBBLICAZIONE.md`, `post-facebook-gruppi-locali.md` — quelli sì aggiornati. Vedi `_handover/APERTI.md` punto 1 per il dettaglio.
- **`instagram-fiducia-proprietari/`**: non ha ancora un `checklist-compliance.md` né una `revisione-marketing-design.md` dedicati (la conformità è stata verificata ma non tracciata in un documento a parte). Vedi `_handover/APERTI.md` punto 2.

## Angoli/ganci già usati (per non ripeterli)

- Rischio affitto tradizionale / morosità / casa sfitta
- Fiducia / trasparenza / "a chi do le chiavi" / come funziona / Q&A
- Costo-opportunità del tempo del fai-da-te (ore/margine invisibile perso, non un confronto tabellare noi/loro)
...
- Superhost in due mesi su una casa non nostra, come prova che precede il prezzo — reel
...
- La prima foto dell'annuncio come cosa che decide il click, con prima/dopo sulla stessa stanza — giornaliero 14/09
- Il calendario della città che decide il prezzo della notte: eventi e ponti dell'autunno romano, i quattro passaggi con cui si costruisce il prezzo, la notte singola che resta vuota fra due prenotazioni — giornaliero 15/09
- Il costo del vuoto: le cinque voci che una casa chiusa paga comunque, date come modulo da compilare invece che come media inventata — giornaliero 16/09
- L'estate non è una prova: «ad agosto sei pieno come tutti, non è lì che si vede il gestore» — il metro di giudizio si sposta su ottobre-marzo, con il confronto agosto/novembre su cinque voci di mestiere (prezzo, durata, ospite, canali, manutenzione) — giornaliero 17/09
...
- **Le recensioni non sono un complimento, sono un prezzo** — non il contenuto delle recensioni ma la loro **resa economica**: il punteggio pesa nel posizionamento dei portali, la visibilità porta prenotazioni, e allora il prezzo non si abbassa; e cambia chi prenota. Prove: recensioni verbatim a 5 stelle (senza nome della casa) e livello Superhost. Dato citabile: Cornell 2012, +1 punto → circa +11% di prezzo, coi suoi caveat — giornaliero 18/09. **Distinto** da `facebook-recensioni-superhost`, che parlava di *cosa dicono* le recensioni
...
1. Crea `campagne/<nome-campagna>/` (segue `campagne/README.md` per la struttura standard).
2. Aggiungi una riga a questo indice appena la campagna ha almeno un angolo definito — anche prima di completarla.
3. Aggiorna la sezione "Angoli/ganci già usati" sopra.
